...
export function preProcess({
  ...
  for (const node of sourceFile.statements) {
    ...
    if (ts.isFunctionDeclaration(node) || ts.isClassDeclaration(node)) {
      ...
      if (isPunctuation) {
        const addSpace =
          code.slice(token.getEnd(), nextToken.getStart()) != " ";
        code.appendLeft(
          nextToken.getStart(),
          `${addSpace ? " " : ""}${defaultExport}`
        );
      } else {
  ...
  /**
   * Transform type-only imports into a form that is compatible with the
   * TypeScript compiler.
   *
   * This transformation is necessary because the TypeScript compiler does not
   * handle type-only imports correctly, and it can lead to issues with the
   * generated types.
   */
  ...
  /**
   * Transform type-only exports into a form that is compatible with the
   * TypeScript compiler.
   *
   * This transformation is necessary because the TypeScript compiler does not
   * handle type-only exports correctly, and it can lead to issues with the
   * generated types.
   */
    ...
    if (
      ...
      for (const element of node.exportClause.elements) {
        ...
        if (node.isTypeOnly || element.isTypeOnly) {
          const localName = uniqName(
            getSafeName((node.moduleSpecifier as ts.StringLiteral).text)
          );
      ...
      if (types.length) {
        ...
        code.overwrite(
          ...
          [
            values.length
              ? `export { ${values.join(", ")} } from ${specifier};`
              : "",
            `import type { ${types
              .map(
                (hint) => `${getNameBinding(hint.sourceName, hint.localName)}`
              )
              .join(", ")} } from ${specifier};`,
            ...types.map(
              (hint) => `type ${hint.hintName} = ${hint.localName};`
            ),
            `export type { ${types
              .map(
                (hint) => `${getNameBinding(hint.localName, hint.targetName)}`
              )
              .join(", ")} };`
          ]
            .filter(Boolean)
