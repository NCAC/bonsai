...
Toute violation detectable sans execution DOIT être une erreur TypeScript.
...
qui garantissent mecaniquement la presence des handlers obligatoires avant la première execution.
...
Un bootstrap reussi garantit la coherence initiale du système — pas de demi-demarrage.
...
le replier lit l'état de son Entity déjà en memoire, aucune notion de
"delai configure" n'a de sens. Si le replier leve une exception, `null` est
également retourne, avec un `console.error` (pas de `RequestError` levee).
...
> des diagnostics impeccables. La rigueur des invariants impose une égale rigueur
