...
| `TProjectionNode` | Type de lecture + mutation N1 sur un @ui DOM, extends `TProjectionRead`. Primitives : `text()`, `attr()`, `toggleClass()`, `visible()`, `style()`, `element(): TEl` (échappatoire typée, I39). Retourné par `getUI(key)` pour les @ui **sans** template. N'expose pas de `.node` (I39, D19, D32) | [view.md](../4-couche-concrete/view.md) |
