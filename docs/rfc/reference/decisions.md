...
| 2026-03-19 | **D42 — VIEW-SUBSCRIPTION : Views s'abonnent via `any` + selectors** ⚠️ **en tension avec ADR-0042 — non tranché** | Les Views s'abonnent aux Channels via l'événement `any` et des selectors namespacés. Le framework auto-gère la souscription et la projection réactive. **[ADR-0042](../../adr/ADR-0042-view-contract-unified-ui-deps-single-generic.md) (livré, ADR-0028 strate 0/1a) a établi à la place des handlers granulaires `on{NS}{Event}Event` par Event déclaré** — l'exact opposé de l'abonnement unique via `any` que D42 décrit. Les deux mécanismes ne coexistent pas aujourd'hui : D42 reste la cible de `5-rendu.md` §7 (strate 1c, templates/selectors), mais l'articulation entre les deux — un même Event routé à la fois vers un handler granulaire et vers un selector `any` ? lequel prévaut ? — n'a pas été tranchée par un ADR dédié. | Abonnement par Event individuel — rejeté car trop verbeux et fragile face à l'ajout de nouveaux Events (position depuis inversée pour le contrat livré par ADR-0042, cf. note ci-dessus) |
