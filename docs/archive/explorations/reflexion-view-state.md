...
Je pense qu'on pourait envisager un "state local" pour N2 et N3 en se disant qu'une View _devrait pouvoir réagir égalament aux changements de son state local_ en plus de réagir aux changements de state d'autre composants. Mais que si on a besoin de broadcaster ces changements, il faut migrer le state local vers un Channel <-> Feature <-> Entity.
...
- **API d'implémentation** (`defineLocalState`, `localState.get()`, `localState.set()`, selector fusion) : détails du _comment_, traités dans RFC-0002 lors de la phase d'implémentation.
